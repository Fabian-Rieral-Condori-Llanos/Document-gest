  ...
  describe('Template Suite Tests', () => {
    ...
    describe('Template CRUD operations', () => {
      ...
      it('Create 2 templates', async () => {
        ...
        template1Id = response.body.data._id
        ...
        var response = await request(app).post('/api/templates')
          ...
          .send(template)
        expect(response.status).toBe(201)
        template2Id = response.body.data._id
      ...
      it('Get templates (existing templates in db)', async () => {
        ...
        expect(response.status).toBe(200)
        expect(response.body.data.map(t => {return {name: t.name}})).toEqual(expect.arrayContaining(expected))
      })
