module.exports = function(request, app) {
  describe('CVSS Suite Tests', () => {
    ...
    beforeAll(async () => {
      ...
      auditId = response.body.data[0]._id
    ...
    describe('CVSS Checks', () => {
      it('Get audit CVSS with both 3.1 and 4.0', async () => {
        ...
        expect(response.status).toBe(200);
        expect(response.body.data.report.public.scoringMethods.CVSS3).toEqual(true);
        expect(response.body.data.report.public.scoringMethods.CVSS4).toEqual(true);
        ...
        expect(response.status).toBe(200)
        expect(response.body.data.findings[0].cvssv3).not.toBeNull()
        expect(response.body.data.findings[0].cvssv4).not.toBeNull()
      })
    })
  })
}