  ...
  describe('Audit Suite Tests', () => {
    ...
    describe('Audit CRUD operations', () => {
      ...
      it('Create audit', async () => {
        ...
        audit1Id = response.body.data.audit._id
      })
      ...
      it('Create second audit', async () => {
        ...
        expect(response.status).toBe(201)
        audit2Id = response.body.data.audit._id
      })
      ...
      it('Get audit general info', async () => {
        ...
        expect(response.body.data.name).toBe('Audit 1');
        expect(response.body.data.auditType).toBe('Web');
        expect(response.body.data.language).toBe('en');
        expect(response.body.data.collaborators).toHaveLength(0);
        expect(response.body.data.reviewers).toHaveLength(0);
        expect(response.body.data.customFields).toHaveLength(0);
        expect(response.body.data.scope).toHaveLength(2);
        expect(response.body.data.scope[0]).toBe('Scope Item 1');
        expect(response.body.data.scope[1]).toBe('Scope Item 2');
      })
    })
  })
}