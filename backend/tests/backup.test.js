    ...
    describe('Backup CRUD operations', () => {
      ...
      const waitForStatusIdle = async (timeout = 30000, interval = 1000) => {
        ...
        while (Date.now() - startTime < timeout) {
          ...
          if (response.body.data.operation === "idle") {
      ...
      it('Should create full backup without POST data', async () => {
        ...
        expect(response.status).toBe(200)
        expect(response2.body.data).toMatchObject({operation: "backup"})
      ...
      it('Should be 3 Backups', async () => {
        ...
        expect(response.body.data[1].name).toBe("Full Backup")
        expect(response.body.data[1].type).toBe("full")
        expect(response.body.data[1].protected).toBe(true)
        expect(response.body.data[1].data).toHaveLength(13)
        expect(response.body.data[2].name).toBe("Partial Backup")
        expect(response.body.data[2].type).toBe("partial")
        expect(response.body.data[2].protected).toBe(false)
        expect(response.body.data[2].data).toEqual(["Audits", "Users"])

        backupRestore = response.body.data[0]
        backupDownload = response.body.data[1]
        backupUpload = response.body.data[2]
      })
      ...
      it('Should be 2 Backups', async () => {
        ...
        expect(response.body.data[0].type).toBe("full")
        expect(response.body.data[0].protected).toBe(false)
        expect(response.body.data[0].data).toHaveLength(13)
        expect(response.body.data[1].name).toBe("Partial Backup")
        expect(response.body.data[1].type).toBe("partial")
        expect(response.body.data[1].protected).toBe(false)
        expect(response.body.data[1].data).toEqual(["Audits", "Users"])
      })
      ...
      it('Should not upload backup with path traversal filename', async () => {
        ...
        expect(response.body.data).toEqual("Invalid characters in filename")
      })
      ...
      it('Should not upload backup with wrong extension', async () => {
        ...
        expect(response.status).toBe(422)
        expect(response.body.data).toEqual("Only .tar archives are allowed")
      })
      ...
      it('Should Restore backup', async () => {
        ...
        expect(response.status).toBe(200)
        expect(response2.body.data).toMatchObject({operation: "restore"})
        await waitForStatusIdle()
