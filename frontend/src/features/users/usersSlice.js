...
const initialState = {
  ...
  loading: false,
  loadingRoles: false,
  togglingUsers: {}, // { [userId]: true/false } para track de usuarios en proceso de toggle
...
const usersSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      // TOGGLE USER ENABLED
      // ============================================
      .addCase(toggleUserEnabled.pending, (state, action) => {
        const userId = action.meta.arg.id;
        state.togglingUsers[userId] = true;
      })
      .addCase(toggleUserEnabled.fulfilled, (state, action) => {
        const { id, enabled } = action.payload;
        state.togglingUsers[id] = false;
        // Actualizar el usuario en la lista
        const index = state.users.findIndex(u => u._id === id);
        if (index !== -1) {
          state.users[index].enabled = enabled;
        }
        // Actualizar selectedUser si es el mismo
        if (state.selectedUser?._id === id) {
          state.selectedUser.enabled = enabled;
        }
      })
      .addCase(toggleUserEnabled.rejected, (state, action) => {
        const userId = action.meta.arg.id;
        state.togglingUsers[userId] = false;
        state.error = action.payload;
      });
  },
});
